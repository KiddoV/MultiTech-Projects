...
Test(neutron, event, var) {
    ;;var := RegExReplace(var, "id=""", "HAHAHAH")
    Clipboard := var
    MsgBox % var
}
...
DisplayProgCard(Result) {
    If (Result.HasNames && Result.HasRows) {
        ...
        NeutronWebApp.qs("#prog-card-modal-container").innerHTML := ""  ;Clear all template modals
        ...
        Loop, % programData.Length()
        {
            ...
            html =
            (Ltrim
            <div id="%progDBId%" type="button" class="prog-card card p-1 pl-2 mb-2 %progStatusClass% fast animated bounceInDown hoverable toggle-modal" style="max-width: 99`%; height: 60px;" data-target="#progCardModal-%progDBId%" onclick="ahk.DisplayProgCardModal(this)">
                <div class="row">
                    <div class="col-md-10">
                        <div class="row">
            ...
            ;;;Create modal for each programcard    (Min-max modal)
            progCardModalHtmlTemplate := NeutronWebApp.wnd.progCardModalTemplate    ;From JS
            ;Modify value in the template
            progCardModalHtmlTemplate := RegExReplace(progCardModalHtmlTemplate, "id=""prog-card-modal""", "id=""progCardModal-" . progDBId . """")
            progCardModalHtmlTemplate := RegExReplace(progCardModalHtmlTemplate, "id=""pills-basic-info""", "id=""pills-basic-info-" . progDBId . """")
            progCardModalHtmlTemplate := RegExReplace(progCardModalHtmlTemplate, "id=""pills-build""", "id=""pills-build-" . progDBId . """")
            progCardModalHtmlTemplate := RegExReplace(progCardModalHtmlTemplate, "id=""pills-pcb""", "id=""pills-pcb-" . progDBId . """")
            progCardModalHtmlTemplate := RegExReplace(progCardModalHtmlTemplate, "href=""#pills-basic-info""", "href=""#pills-basic-info-" . progDBId . """")
            progCardModalHtmlTemplate := RegExReplace(progCardModalHtmlTemplate, "href=""#pills-build""", "href=""#pills-build-" . progDBId . """")
            progCardModalHtmlTemplate := RegExReplace(progCardModalHtmlTemplate, "href=""#pills-pcb""", "href=""#pills-pcb-" . progDBId . """")
            NeutronWebApp.qs("#prog-card-modal-container").insertAdjacentHTML("beforeend", progCardModalHtmlTemplate)
        }
    }
}
...
DisplayProgCardModal(neutron, event) {
    ...
    ;;;Save data to var
    ;;Table 1
    pcmProgDBId := "progCardModal-" . programData[1].prog_id
    ...
    If (pcmProgSubsName = "") {
        NeutronWebApp.qs(pcmProgDBId . " [name='prog-card-modal-title']").innerHTML := pcmProgName . isAlternate
        ;NeutronWebApp.wnd.getBackOldModalCardProgInfoContent()      ;JS function
    } Else {
        NeutronWebApp.qs(pcmProgDBId . " [name='prog-card-modal-title']").innerHTML := pcmProgName . isAlternate . " <span class='red-text'>(USE => " . pcmProgSubsName . ")</span>"
        NeutronWebApp.qs(pcmProgDBId . " [name='prog-card-main-info-group']").innerHTML := "<h6 class='prog-card-title red-text'>This program is the same with " . pcmProgSubsName . "<br>So ...Use it instead!</h6> <h6 class='prog-card-title'>Noted that program <span class='red-text'>" . pcmProgSubsName . "</span> might need ECO updates.<br>Make sure to verify that before you use!</h6>"
    }
    NeutronWebApp.qs(pcmProgDBId . " [name='prog-card-modal-header']").style.backgroundColor := progStatusColor
    NeutronWebApp.qs(pcmProgDBId . " [name='prog-card-modal-buildnum']").innerHTML := pcmProgBuildNum
    NeutronWebApp.qs(pcmProgDBId . " [name='prog-card-modal-eclnum']").innerHTML := pcmProgCurntEcl
    NeutronWebApp.qs(pcmProgDBId . " [name='prog-card-modal-econum']").innerHTML := pcmProgCurntEco
    NeutronWebApp.qs(pcmProgDBId . " [name='prog-card-modal-pcbnum']").innerHTML := pcmProgPcbNum
    ;NeutronWebApp.qs("#prog-card-modal-pcb-btn").innerHTML := pcmProgPcbNum = "" ? "????????" : pcmProgPcbNum
    NeutronWebApp.qs(pcmProgDBId . " [name='prog-card-stat-label']").innerHTML := "<span class='badge' style='background-color: " . progStatusColor . "'>" . pcmProgStatus . "</span>"
    NeutronWebApp.qs(pcmProgDBId . " [name='prog-card-modal-brand-logo']").src := brandLogoPath
    NeutronWebApp.qs(pcmProgDBId . " [name='prog-card-rtf-note']").innerHTML := "<a href='#' onclick='ahk.OpenRTFNote(event)'>Notes.rtf</a>"
    NeutronWebApp.qs(pcmProgDBId . " [name='prog-card-date-created']").innerHTML := dateCreated
    NeutronWebApp.qs(pcmProgDBId . " [name='prog-card-alt-type']").innerHTML := isAlternate := pcmProgAltType = "" ? "<span class='badge mdb-color'>ORIGINAL</span>" : "<span class='badge black'>" . pcmProgAltType . "</span>"
    NeutronWebApp.qs(pcmProgDBId . " [name='prog-card-created-by']").innerHTML := pcmUserFn . " " . pcmUserLn
    ...
    NeutronWebApp.qs(pcmProgDBId . " [name='prog-card-modal-build-name']").innerHTML := pcmBuildName . "  <span class='pricetag z-depth-1 default-mouse'>$" . pcmBuildCost . "</span>"
    NeutronWebApp.qs(pcmProgDBId . " [name='prog-card-modal-build']").innerHTML := pcmProgBuildNum
    NeutronWebApp.qs(pcmProgDBId . " [name='prog-card-modal-build-status']").innerHTML := "Status: <span class='badge " . buildStatusColor . "'>" . pcmBuildStatus . "</span>"
    NeutronWebApp.qs(pcmProgDBId . " [name='prog-card-modal-build-instock']").innerHTML := pcmBuildQnty . " (pcs)"
    NeutronWebApp.qs(pcmProgDBId . " [name='prog-card-modal-build-eco-hist']").innerHTML := ""     ;Delete all old result before display new result
    ...
    If (ProgCardBuildECOHistData.HasRows) {
        Loop, % ProgCardBuildECOHistData.RowCount
        {
            ...
            NeutronWebApp.qs(pcmProgDBId . " [name='prog-card-modal-build-eco-hist").insertAdjacentHTML("beforeend", html)
        }
    }
    ...
    NeutronWebApp.qs(pcmProgDBId . " [name='prog-card-modal-pcb-name']").innerHTML := pcmPcbFullName " | " pcmProgPcbNum
    NeutronWebApp.qs(pcmProgDBId . " [name='prog-card-modal-pcb']").innerHTML := pcmProgPcbNum
    NeutronWebApp.qs(pcmProgDBId . " [name='prog-card-modal-pcb-status']").innerHTML := "Status: <span class='badge " . pcbStatusColor . "'>" . pcmPcbStatus . "</span>"
    NeutronWebApp.qs(pcmProgDBId . " [name='prog-card-modal-pcb-dwg']").innerHTML := pcmPcbDwg
    NeutronWebApp.qs(pcmProgDBId . " [name='prog-card-modal-pcb-quant']").innerHTML := pcmPcbQty . " (pcs)"
    NeutronWebApp.qs(pcmProgDBId . " [name='prog-card-ins-file']").innerHTML := "<a href='#' onclick='ahk.OpenInsFile(event, """ . pcmProgDBId . """)'>" . pcmPcbInsFileName . "</a>"
    ;NeutronWebApp.qs(pcmProgDBId . " [name='prog-card-ins-file']").innerHTML := "<a href='http://virtu.multitech.prv:4080/Data/PCB_Assembly_Data/" . pcmPcbInsFileName . "' target='_blank'>" . pcmPcbInsFileName . "</a>"
    NeutronWebApp.qs(pcmProgDBId . " [name='prog-card-assem-draw']").innerHTML := "<a href='#' onclick='ahk.OpenAssemDraw(event)'>" . pcmProgPcbNum . "_assemblyDrawing.PDF</a>"
}
...
OpenInsFile(neutron, event, progModalId) {
    insFileToOpen := event.target.innerHTML
    insFileUrl := "http://virtu.multitech.prv:4080/Data/PCB_Assembly_Data/" . insFileToOpen
    Clipboard := insFileUrl
    NeutronWebApp.qs(progModalId . " [name='prog-card-ins-file']").innerHTML := "Openning..."
    insTempFilePath := A_MyDocuments . "\insTemp.txt"
    URLDownloadToFile, %insFileUrl%, %insTempFilePath%
    MsgBox % insTempFilePath
    If (ErrorLevel = 0)
        Run,  %insTempFilePath%, , Hide
    Else
        DisplayAlertMsg("Could not open Insert file! COMPFIND not connected!", "alert-danger")
        
    NeutronWebApp.qs(progModalId . " [name='prog-card-ins-file']").innerHTML := "<a href='#' onclick='ahk.OpenInsFile(event, """ . progModalId . """)'>" . insFileToOpen . "</a>"
}
