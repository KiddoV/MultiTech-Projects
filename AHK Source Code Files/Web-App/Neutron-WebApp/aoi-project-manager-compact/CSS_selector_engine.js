		...
		while(current_PseudoClassCheckObject = pseudoClassCheckObjects[++iterateIndex]) {
			...
			if(_tmp2 < 2 && current_PseudoClassCheckObject[2]) {
				if(!RE__not_a_number.test(current_PseudoClassCheckObject[2])) { // number value like nth-child(2n+1)
					...
					if(!current_PseudoClassCheckObject[2][3]) {//delete noop nth-child(n)
					...
					while(
						...
						switch(css3PseudoType) {
							...
							case 1://'nth-last-child': // W3C: "an E element, the n-th rs of its parent, counting from the last one"
								// current_PseudoClassCheckObject[1] - pseudo type defined in map SELECTOR_PSEUDOS_MAP
								// current_PseudoClassCheckObject[2] - parsed Tree-Structural pseudo-class rule, for example in :nth-child(an+b) "an+b" - is a rule. Structure for rule:
								// {
								// 0 : undefined // not using
								// 1 : <number> // "a" from "an+b"
								// 2 : <string> // operator "an+b", in this case it is "+"
								// 3 : <number> // "b" from "an+b"
								// }
								// current_PseudoClassCheckObject[3] - property for saving caching result of rule (for example "an+b") to Node "nodeIndexLast" or "nodeIndex"
								// current_PseudoClassCheckObject[4] - Node child property name: "lastChild" or "firstChild"
								// current_PseudoClassCheckObject[5] - Node sibling property name: "previousSibling" or "nextSibling"
